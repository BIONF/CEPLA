...
class PopulationCoverage(object):
    ...
    def calculate_coverage(self, query_by="area_country_ethnicity", population=None, mhc_class=None, filename=None, user_defined_population_data_lines=None):
        ...
        """
        """ EDITED PART TO GET INDIVIDUAL PERCENTAGES FOR EVERY EPITOPE"""

        every_epitope = {}
        all_epitopes_copy = [i for i in self.input_epitope_allele_list]
        marker = False
        for i in all_epitopes_copy:
            self.input_epitope_allele_list = [i]
                ...
                for mhc_class, coverage_data in class_map.items():
                    ...
                    if len(result_map) > 1:
                        ...
                        if frequency_map:
                            frequency_map.update({"mhc_class": mhc_class})
                            frequency_map.update({"population": population})
                        every_epitope[i[0]] = round(into_percentage(float(frequency_map["coverage"])), 2)




        self.input_epitope_allele_list = [i for i in all_epitopes_copy]

        """ EDITED PART ENDS HERE - RETURN VALUE MODIFIED """

        return result, negative, every_epitope #Modified
